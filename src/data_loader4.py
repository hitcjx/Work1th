...
class SquatDataset(Dataset):
    def __init__(self, csv_file, keypoints_folder, transform=None):
        ...
        with mp_pose.Pose(min_detection_confidence=0.5, min_tracking_confidence=0.5) as pose:
            for key_frame in key_frames:
                if key_frame['file_id'] == video_file.split('/')[-1]:
                    ...
                    # 检查读取到的帧
                    print(
                        f"Frame at {time_ms}ms: Shape = {frame.shape}, dtype = {frame.dtype}"
                    )
...
def random_rotate(keypoints):
    ...
    for i in range(17):
        ...
        keypoints[i * 3], keypoints[i * 3 + 1] = rotated[0], rotated[1]
...
def random_translate(keypoints):
    ...
    keypoints[0::3] += x_translation  # 对所有 x 坐标进行平移
    keypoints[1::3] += y_translation  # 对所有 y 坐标进行平移
    return keypoints
