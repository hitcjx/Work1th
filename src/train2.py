...
model.output_heads = nn.ModuleList([
    nn.Sequential(nn.Linear(1024, 6)),  # error_type 输出头
    nn.Sequential(nn.Linear(1024, 1)),  # valid_squat 输出头
])
...
for epoch in range(num_epochs):
    ...
    # 使用 tqdm 显示进度条
    for i, (keypoints, action_stage, error_type, valid_squat) in enumerate(tqdm(train_dataloader, desc=f"Epoch {epoch+1}/{num_epochs}")):
        # 将数据移动到设备
        ...
        error_type_output = model.output_heads[0](model.features(keypoints))
        valid_squat_output = model.output_heads[1](model.features(keypoints))
